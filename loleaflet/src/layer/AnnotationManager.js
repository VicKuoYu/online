	...
	layout: function () {
		...
		if (this._selected) {
			...
			for (var idx = selectIndexFirst + 1; idx <= selectIndexLast; idx++) {
				latlng = this._map.unproject(layoutBounds.getBottomLeft());
				// FIXME: Setting latlng and animating these comments make them dissappear from the screen.
				this._animation.run(this._items[idx]._container, this._map.latLngToLayerPoint(latlng));
